# ADR-007 : Parité des Caractéristiques et Modèles "Lean" pour la Production (Train/Serve Skew)

Lors du passage de la phase de validation à l'intégration au sein de l'architecture SOC en temps réel, une disparité technique majeure a été identifiée entre les données d'entraînement (dataset CICIDS2017) et les données générées en direct par le moteur IDS Suricata dans notre infrastructure.
...
Utiliser les modèles v1 tels quels en production créerait un décalage : le modèle recevrait des requêtes avec des features manquantes (remplies par des zéros), ce qui fausserait complètement les prédictions d'anomalies.

Nous avons opté pour une approche MLOps de "Feature Parity" (Parité des Caractéristiques).

Nous avons décidé de maintenir **deux versions distinctes** des modèles de Machine Learning :
1. **Modèles v1 (Benchmark Offline) :** Entraînés sur les 20 features originelles de CICIDS2017. Ils servent de référence (Baseline) pour documenter les performances maximales théoriques dans la phase de recherche du projet.
